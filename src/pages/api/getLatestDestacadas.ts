...
export default async function getLatestDestacadas(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      ...
      const query = `SELECT * FROM \`${datasetId}.${tableId}\` ORDER BY date DESC LIMIT 5`;
      const [rows] = await bigQueryClient.query({ query });

      res.status(200).json(rows);
    } catch (error) {
      console.error('Error al obtener las ultimas 5 destacadas de BigQuery:', error);
      res.status(500).json({ error: 'Error al obtener las ultimas 5 destacadas' });
    }
  } else {
